...
class VeriChain {
  ...
  async connectWallet() {
    ...
    walletAddress.innerHTML = `<span title="Click to copy full address" style="cursor: pointer;">${this.formatAddress(
      address
    )}</span>`;
    ...
    resultDiv.innerHTML = `
            <div style="display: flex; align-items: center; gap: 1rem;">
                ...
                <div>
                    ...
                    <p><strong>ID: ${
                      credential.id
                    }</strong> | Transaction: <a href="#" onclick="return false;">${this.formatHash(
      credential.txHash
    )}</a></p>
                </div>
            </div>`;
  ...
  updateStudentDashboard() {
    ...
    const userCredentials = this.credentials.filter(
      (c) =>
        c.studentWallet.toLowerCase() === this.connectedWallet.toLowerCase()
    );
    ...
    if (userCredentials.length === 0) {
      credentialsDiv.innerHTML = `
                <div class="empty-state">
                    ...
                    <p>Credentials issued to <code>${this.formatAddress(
                      this.connectedWallet
                    )}</code> will appear here.</p>
                </div>`;
    } else {
      credentialsDiv.innerHTML = userCredentials
        .map(
          (cred) => `
                <div class="credential-card">
                    ...
                    <div class="detail-grid">
                        ...
                        <div><strong>Issued:</strong> ${new Date(
                          cred.timestamp
  ...
  updateStats() {
    document.getElementById("credentialsIssued").textContent =
  ...
  getFormData(formId) {
    const data = {};
    new FormData(document.getElementById(formId)).forEach(
